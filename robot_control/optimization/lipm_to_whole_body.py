...
def compute_foot_traj(foot_steps, N, dt, step_time, step_height, first_phase):
    ...
    if first_phase=='swing':
        ...
        x[1, :N_step] = foot_steps[1,0]
        
    for s in range(foot_steps.shape[1]):
        i = offset+s*2*N_step
        x[0, i:i+N_step] = foot_steps[0,s]
        x[1, i:i+N_step] = foot_steps[1,s]
        if s<foot_steps.shape[1]-1:
...
def interpolate_lipm_traj(T_step, nb_steps, dt_mpc, dt_ctrl, com_z, g,
    ...
    foot_steps = np.empty((4,N_ctrl+1))*np.nan
    com[2,:] = com_z
    
    N_inner = int(N_ctrl/N)
    for i in range(N):
        com[0,i*N_inner] = com_state_x[0,i]
        com[1,i*N_inner] = com_state_y[0,i]
        dcom[0,i*N_inner] = com_state_x[1,i]
        dcom[1,i*N_inner] = com_state_y[1,i]
    ...
    return com, dcom, ddcom, cop, foot_steps
...
if __name__=='__main__':
    ...
    for foot_name in conf.ee_frames:
        ...
        target_foot_steps = p[indices, shift::2]
        x[foot_name], dx[foot_name], ddx[foot_name] = compute_foot_traj(
            target_foot_steps, N_ctrl, dt_ctrl, conf.T_step, conf.step_height, initial_phase)
    ...
    for i in range(2):
        ...
        plt.plot(time, com_state[i, :-1], ":", label="CoM TO")
        plt.legend()
