...
- Now you have two options: 
  ...
  ```
  docker pull mfocchi/trento_lab_framework:wheresim 
  ```
  ...
  docker build -t mfocchi/trento_lab_framework:wheresim -f Dockerfile .
  ```
...
**NOTE!** If you do not have an Nvidia card in your computer, you should skip the parts about the installation of the drivers, and you can still run the docker **without** the **--gpus all**  in the **lab_wheresim** alias.
...
- the **lab_wheresim** script will mount the folder `~/trento_lab_home` on your **host** computer. Inside of all of the docker images this folder is mapped to `$HOME`.This means that any files you place   in your docker $HOME folder will survive the stop/starting of a new docker container. All other files and installed programs will disappear on the next run. 
- The alias **lab_wheresim** needs to be called only ONCE and opens the image. To link other terminals to the same image you should run **dock-other**, this second command will "**attach**" to the image opened previously by calling the **lab_wheresim** alias.  You can call **lab_wheresim** only once and **dock-other** as many times you need to open multiple terminals.
...
```powershell
$ docker commit ASH mfocchi/trento_lab_framework:wheresim
```
